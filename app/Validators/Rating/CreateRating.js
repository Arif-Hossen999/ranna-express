'use strict'
/**
 ** File Name: CreateRating.js
 ** Validating Rating Info for adding new Rating
...
class CreateRating {
   ...
   */
  get rules() {
    const { id } = this.ctx.request.input('ratingInfo')
    return {
      // validation rules
      'ratingInfo.ratings': `required`,
    }
  }
  /**
  ...
  get messages() {
    ...
    return {
      'ratingInfo.ratings.required': antl.formatMessage(
        'validations.required',
        {
          field: antl.formatMessage('keys.rating'),
        }
      ),
    }
  }
